...
export class ArchivedWorkflowList extends BasePage<RouteComponentProps<any>, State> {
    ...
    public render() {
        return (
            <Page
                ...
                toolbar={{
                    breadcrumbs: [{title: 'Archived Workflows', path: '/archived-workflows'}]
                }}>
                <div className='row'>
    ...
    private saveHistory() {
        this.url = '/archived-workflows/' + (this.state.namespace || '') + '?' + this.filterParams.toString();
            ...
            <>
                <div className='argo-table-list'>
                    ...
                    {this.state.workflows.map(w => (
                        <Link className='row argo-table-list__row' key={`${w.metadata.uid}`} to={`/archived-workflows/${w.metadata.namespace}/${w.metadata.uid}`}>
                            <div className='columns small-1'>
