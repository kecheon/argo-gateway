...
export class CronWorkflowService {
    ...
    public suspend(name: string, namespace: string) {
        return requests.put(`/argo/cron-workflows/${namespace}/${name}/suspend`).then(res => res.body as CronWorkflow);
    }

    public resume(name: string, namespace: string) {
        return requests.put(`/argo/cron-workflows/${namespace}/${name}/resume`).then(res => res.body as CronWorkflow);
    }
}
