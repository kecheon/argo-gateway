...
var Identity = exports.Identity = function (options) {
  ...
  //self.basePath = options.basePath || '/v3/auth/tokens';
    self.basePath = options.url;
...
Identity.prototype.authorize = function (options, callback) {
  ...
  var authenticationOptions = {
    uri: new URL(options.url || self.options.url, self.basePath),
    method: 'POST',
    strictSSL: options.strictSSL || self.options.strictSSL,
    headers: {
      'User-Agent': util.format('nodejs-pkgcloud/%s', 'custom pkgcloud'),
  ...
  if (self.options.version === 1 || self.options.version === '/v1.0') {
    authenticationOptions.uri = new URL(options.url || self.options.url, '/auth/v1.0');
  ...
  delete self._authenticationPayload;
    ...
    try {
      ...
      // go ahead and make a 1-off request to get a tenant and then reauthorize
        ...
        getTenantId(new URL(options.url || self.options.url, '/v2.0/tenants'), body.access.token.id);
      }
      else {
